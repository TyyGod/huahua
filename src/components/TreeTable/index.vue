  ...
  <div ref="treetable" class="tree-table">
    <div class="tree-table-header" :style="{width: getTreeTableTrWidth + 'px', paddingRight: (getTreeTableContentIsScroll ? getScrollbarWidth : 0) + 'px'}">
      <template v-for="(item, index) in getColumn">
        <div :key="index" class="tree-table-header-th" :style="{width: item.width + 'px'}">
          <div class="cell">{{ item.title }}</div>
        </div>
      </template>
      <div v-if="operationOpen" class="tree-table-header-th" :style="{width: getOperWidth + 'px'}">
        <div class="cell">{{ operationText }}</div>
      </div>
    </div>
    <div ref="treetablecontent" class="tree-table-content" :style="{width: getTreeTableTrWidth + 'px'}">
      <el-tree
        ref="el-tree"
        :data="data"
      ...
      >
        ...
        <span slot-scope="{ node, data }" class="custom-tree-node">
          <template v-for="(item, index) in getColumn">
            <span :key="index" class="custom-tree-node-td" :style="{width: setNodeTdWidth(index, node, item.width) + 'px'}">
    ...
    <div class="tree-table-bottom-border" :style="{width: getTreeTableTrWidth + 'px'}" />
  </div>
</template>
<script>
/**
  ...
  computed: {
    ...
    getColumTableWidthDelOperWidth: function() {
      ...
      for (let index = 0; index < _this.getColumnLen; index++) {
        const width = _this.column[index]['minWidth'] ? _this.column[index]['minWidth'] : 100
    ...
    getColumn: function() {
      ...
      for (let index = 0; index < _this.getColumnLen; index++) {
        ...
        const actualWidth = _this.getTreeTableDelOperWidth > _this.getColumTableWidthDelOperWidth ? (_this.getTreeTableDelOperWidth * width / _this.getColumTableWidthDelOperWidth) : width
        newColum.push({
          title: item['title'],
          label: item['label'],
          width: actualWidth
